...
| Status | Implementation complete and locally verified — draft PR #2 open, CI retry pending after wrapper-mode fix |
...
| 1 | F01 / US001–US003 | `codex/f01-market-context` | Runnable store shell with persistent market, language, and display-currency context | Foundation | Draft PR #2 open — CI retry pending |
...
| Pull request / CI | Implementation commit `75a5465` pushed; draft PR [#2](https://github.com/IslamDomyaty/mgparts/pull/2) open; initial run `30835115818` failed before Maven because `mvnw` lacked the Git executable bit; wrapper mode corrected and retry pending |
