...
| Verification | `.\mvnw.cmd -B -ntp verify` passed: 16 unit/contract tests and 7 Spring MVC integration tests; executable JAR packaged; rendered browser checks passed for Arabic RTL, English LTR, currency persistence and GB market defaults; `git diff --check` passed; GitHub Actions run `30835224852` passed |
...
| Pull request / CI | Implementation commit `75a5465` pushed; draft PR [#2](https://github.com/IslamDomyaty/mgparts/pull/2) open; initial run `30835115818` failed before Maven because `mvnw` lacked the Git executable bit; fix commit `15ad3e5` corrected the mode and run `30835224852` passed |
